...
async function createCheckoutSession(priceId: string): Promise<string | null> {
  ...
  if (!session) {
    console.error("[Stripe] âŒ No auth session found - user not logged in");
    throw new Error("You must be logged in to purchase premium");
  }
  
  console.log("[Stripe] âœ… Auth session found");
  console.log("[Stripe] ðŸ“¡ Invoking create-checkout edge function...");
  ...
  console.log("[Stripe] ðŸ“¥ Edge function response:", res);
  
  if (res.error) {
    console.error("[Stripe] âŒ Edge function returned error:", res.error);
    console.error("[Stripe] âŒ Error details:", JSON.stringify(res.error, null, 2));
  ...
  if (!res.data?.url) {
    console.error("[Stripe] âŒ No checkout URL in response");
    console.error("[Stripe] âŒ Full response data:", JSON.stringify(res.data, null, 2));
    throw new Error("Edge function did not return a checkout URL");
  }
  
  console.log("[Stripe] âœ… Checkout URL received:", res.data.url);
  return res.data.url;
}
...
export default function PremiumPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("success") === "true") {
      console.log("[Premium] âœ… Payment success detected, refreshing subscription...");
      
      // Force refresh subscription immediately
      refreshSubscription().then(() => {
        console.log("[Premium] âœ… Subscription refreshed");
        
        const from = new Date().toLocaleDateString();
        const to = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString();
        addNotification("ðŸŽ‰ Premium Activated", `Your Premium subscription is active from ${from} to ${to}.`);
        toast.success("ðŸŽ‰ Payment successful! Premium is now active.");
        ...
        setTimeout(() => {
          console.log("[Premium] ðŸ”„ Second refresh to ensure webhook completed");
          refreshSubscription();
        }, 2000);
      });
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      console.log("[Stripe] Yearly price ID:", yearlyPriceId || "(not set â€” will use monthly)");
      ...
      if (!hasPrice) {
        console.warn("[Stripe] âš ï¸ No valid Stripe price ID configured!");
        console.warn("[Stripe] âš ï¸ Falling back to DEMO MODE (direct database update)");
        console.warn("[Stripe] âš ï¸ This will NOT process real payments!");
        toast.warning("Demo mode active - no real payment processed");
      }

      if (hasPrice) {
        console.log("[Stripe] âœ… Valid price ID found, creating Stripe checkout session...");
        try {
          const checkoutUrl = await createCheckoutSession(priceId);
          if (checkoutUrl) {
            console.log("[Stripe] âœ… Checkout URL received:", checkoutUrl);
            console.log("[Stripe] ðŸ”„ Redirecting to Stripe Checkout...");
            toast.info("Redirecting to secure checkout...");
            window.location.href = checkoutUrl;
            return; // Stop execution after redirect
          } else {
            console.error("[Stripe] âŒ Edge function returned success but no URL");
            toast.error("Failed to create checkout session. Check console for details.");
            return;
          }
        } catch (checkoutError) {
          console.error("[Stripe] âŒ Checkout session creation failed:", checkoutError);
          toast.error(`Checkout failed: ${(checkoutError as Error).message}`);
          return;
        }
      } else {
        // Demo mode fallback - direct database update (no Stripe)
        console.log("[Stripe] ðŸŽ­ DEMO MODE: Directly updating subscription in database");
        ...
        addNotification("ðŸŽ‰ Premium Activated (Demo)", `Your ${planLabel} Premium is active from ${from} to ${to}.`);
        toast.success("Demo: Premium activated without payment");
        navigate("/");
      }
    } catch (err) {
      console.error("[Stripe] âŒ Upgrade error:", err);
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      ...
                      {subscription.cancelAtPeriodEnd
                        ? ` Â· Active until ${new Date(subscription.currentPeriodEnd!).toLocaleDateString()}`
        ...
        <div className="relative rounded-2xl border-2 border-primary/50 bg-card p-6 shadow-lg transition-all hover:shadow-xl animate-in zoom-in-95 duration-300">
          ...
          {billing === "yearly" && (
            <p className="text-xs text-income font-medium mb-5">
              ${yearlyMonthly}/month â€” save ${MONTHLY_PRICE * 12 - YEARLY_PRICE}/year
            </p>
          )}
