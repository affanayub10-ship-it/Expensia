...
async function createCheckoutSession(userId: string, priceId: string): Promise<string | null> {
  ...
  if (!session) {
    console.error("[Stripe] âŒ No auth session");
  ...
  console.log("[Stripe] ðŸ“¥ Response:", res);
  
  if (res.error) {
    console.error("[Stripe] âŒ Error:", res.error);
    throw new Error(res.error.message || "Edge function failed");
  }
  
  if (!res.data?.url) {
    console.error("[Stripe] âŒ No URL in response");
    throw new Error("No checkout URL returned");
  }
  
  console.log("[Stripe] âœ… Checkout URL:", res.data.url);
  return res.data.url;
}
...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("success") === "true") {
      console.log("[Pricing] âœ… Payment success detected, refreshing subscription...");
      
      // Force refresh subscription immediately
      refreshSubscription().then(() => {
        console.log("[Pricing] âœ… Subscription refreshed");
        ...
        setTimeout(() => {
          console.log("[Pricing] ðŸ”„ Second refresh to ensure webhook completed");
          refreshSubscription();
        }, 2000);
      });
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (!hasPrice) {
        console.warn("[Stripe] âš ï¸ DEMO MODE: No valid Stripe price ID");
        toast.warning("Demo mode - no real payment");
        
        // Demo mode â€” directly set premium
        ...
        const from = new Date().toLocaleDateString();
        const to = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString();
        addNotification("ðŸŽ‰ Premium Activated (Demo)", `Your Premium is active from ${from} to ${to}.`);
      ...
      console.log("[Stripe] âœ… Creating Stripe checkout session...");
      const checkoutUrl = await createCheckoutSession(user.id, priceId);
      if (checkoutUrl) {
        console.log("[Stripe] âœ… Redirecting to:", checkoutUrl);
      ...
      console.error("[Stripe] âŒ Error:", err);
    ...
    <>
      ...
      <div className="mx-auto max-w-3xl">
              ...
              {isPremium ? (
                <div className="space-y-2">
                  <div className="rounded-xl bg-income/10 px-3 py-2.5 text-xs text-income flex items-center gap-2">
                    ...
                    Premium is active â€” all features unlocked!
                  </div>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          Secure payments powered by Stripe Â· Cancel anytime Â· No hidden fees
        </p>
      </div>
    </>
  );
