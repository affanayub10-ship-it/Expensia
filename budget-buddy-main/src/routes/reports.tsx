    ...
    <>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card className="shadow-soft"><CardContent className="flex items-center gap-3 p-5">
          ...
          <div className="min-w-0"><p className="text-xs text-muted-foreground">Highest Expense</p><p className="truncate font-semibold">{highest?.title??"—"}</p><p className="text-sm text-expense">{highest?formatMoney(highest.amount):""}</p></div>
        </CardContent></Card>
        <Card className="shadow-soft"><CardContent className="flex items-center gap-3 p-5">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="shadow-soft lg:col-span-2">
          <CardHeader><CardTitle className="text-base">Income vs Expenses — {rangeLabel}</CardTitle></CardHeader>
          <CardContent>
              ...
              byCategory.map((c,i)=>{const pct=totalExpenses>0?(c.value/totalExpenses)*100:0; return (
                <div key={c.name}>
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">{formatMoney(c.value)} · {formatPercent(pct)}</span>
                  </div>
            ...
            {selectedCategory && categoryStats && (
              <>
                ...
                <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <Card className="shadow-soft"><CardContent className="flex items-center gap-3 p-5">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-expense/10 text-expense"><Trophy className="h-5 w-5" /></div>
                    <div className="min-w-0"><p className="text-xs text-muted-foreground">Highest Expense</p><p className="truncate font-semibold">{categoryStats.highestExpense?.title??"—"}</p><p className="text-sm text-expense">{formatMoney(categoryStats.highestExpense?.amount||0)}</p></div>
                  </CardContent></Card>
                  <Card className="shadow-soft"><CardContent className="flex items-center gap-3 p-5">
                    ...
                    <div className="min-w-0"><p className="text-xs text-muted-foreground">Lowest Expense</p><p className="truncate font-semibold">{categoryStats.lowestExpense?.title??"—"}</p><p className="text-sm text-income">{formatMoney(categoryStats.lowestExpense?.amount||0)}</p></div>
                  </CardContent></Card>
                  <Card className="shadow-soft"><CardContent className="flex items-center gap-3 p-5">
                ...
                {categorySpendingChart.length > 0 && (
                  <Card className="mb-6 shadow-soft">
                    <CardHeader><CardTitle className="text-base">Spending Over Time — {selectedCategory}</CardTitle></CardHeader>
                    <CardContent>
                ...
                {/* Transaction List */}
                <Card className="shadow-soft">
                  <CardHeader><CardTitle className="text-base">Transactions — {selectedCategory} ({dateRangeLabel})</CardTitle></CardHeader>
                  <CardContent>
